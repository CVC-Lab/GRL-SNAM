#!/usr/bin/env python3
"""
Compare a Stage-1 learned policy against a sampling-based Ackermann MPC baseline.

The comparison uses the same procedural scenes and Ackermann rollout. The policy
receives synthetic-LaserScan-derived local tokens. The MPC baseline uses the
procedural obstacle geometry as a model-aware upper-bound controller.

Usage
-----
python -m compare_stage1_policy_mpc \
  --ckpt checkpoints/stage1_ackermann/best.pt \
  --episodes 20 --plot-count 5 --outdir eval_policy_vs_mpc
"""
...
from eval_stage1_ackermann import load_model, make_policy_fn
...
def save_comparison_plot(
    ...
    p_tr = policy_ro["traj"]
    m_tr = mpc_ro["traj"]
    ax0.plot(p_tr[:, 0], p_tr[:, 1], lw=2.0, label="policy")
    ...
    ax1.plot(np.arange(len(p_dist)), p_dist, label="policy")
    ...
    ax2.plot(np.arange(len(p_clr)), p_clr, label="policy")
    ...
    m_cmd = mpc_ro["cmds"]
    if len(p_cmd):
        ax3.plot(np.arange(len(p_cmd)), p_cmd[:, 0], label="policy v")
        ax3.plot(np.arange(len(p_cmd)), p_cmd[:, 2], label="policy steering")
    if len(m_cmd):
...
def main():
    ap = argparse.ArgumentParser("Compare Stage-1 policy against sampling MPC")
    ap.add_argument("--ckpt", type=str, required=True)
    ap.add_argument("--episodes", type=int, default=20)
    ap.add_argument("--max-steps", type=int, default=160)
    ap.add_argument("--goal-tol", type=float, default=0.35)
    ap.add_argument("--outdir", type=str, default="eval_policy_vs_mpc")
    ...
    policy_fn = make_policy_fn(model, args.device)
    mpc_cfg = MPCConfig(
    ...
    for i in range(args.episodes):
        ...
        # Use separate rollout RNGs so sensor noise/dropout are reproducible per controller.
        ...
        if i < args.plot_count:
            title = (
                f"ep={i} | policy success={pro['success']} final={pro['final_dist']:.2f} "
                f"clear={pro['min_clearance']:.2f} | MPC success={mro['success']} "
                f"final={mro['final_dist']:.2f} clear={mro['min_clearance']:.2f}"
            )
            save_comparison_plot(os.path.join(args.outdir, f"compare_ep_{i:03d}.png"), scene, pro, mro, cfg, title)
    ...
    summary = {
        ...
        "note": "MPC uses procedural obstacle geometry and should be interpreted as a model-aware upper-bound baseline.",
    }
    with open(os.path.join(args.outdir, "summary_policy_vs_mpc.json"), "w") as f:
        json.dump(summary, f, indent=2)
    with open(os.path.join(args.outdir, "episodes_policy.json"), "w") as f:
        json.dump(policy_metrics, f, indent=2)
    with open(os.path.join(args.outdir, "episodes_mpc.json"), "w") as f:
        json.dump(mpc_metrics, f, indent=2)
